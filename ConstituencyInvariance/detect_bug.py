...
from ConstituencyInvariance.SynonymsReplacer import SynonymsReplacer
from ConstituencyInvariance.TransError import TransError
from ConstituencyInvariance.Translation import Translation
import argparse
import re
from LAC import LAC
import jieba
...
def read_stop_words():
    global stop_words
    #add encoding='utf-8'
    f = open('./ConstituencyInvariance/tools/stop_words.txt', mode='r', encoding='utf-8')
    line = f.readline()
    while line:
        line = line[:-1]
...
def load_china_city():
    global city_names
    f = open('./ConstituencyInvariance/tools/china_city.txt', mode='r', encoding='utf-8')
    line = f.readline()
    while line:
        line = line[:-1]
...
def load_china_name():
    global people_names
    f = open('./ConstituencyInvariance/tools/china_people.txt', mode='r', encoding='utf-8')
    line = f.readline()
    while line:
        line = line[:-1]
...
def load_trans_sent(file_name):
    ...
    while line:
        if "sent_id" in line:
            ...
            group_list = []
            step_list = []
            group_list.append(line[:-1])  # add s0
            line = f.readline()
...
def test_sent(test_file, bug_file, s_id, e_id):
    ...
    f = open(bug_file, mode="a",encoding='utf-8')
...
def detect(file_name, s_id, e_id):
    mt_sys = 'youdao'
    translation_file = './Data/Experiment_MT/' + file_name + '/' + file_name + "_" + mt_sys + '_translation.txt'
    bug_file = './Data/Experiment_MT/' + file_name + '/' + file_name + "_" + mt_sys + '_bug.txt'
    read_stop_words()
    load_china_city()
    load_china_name()
